...
const ecgData = ref<{ time: number; displayTime: number; ecg: number }[]>([])
...
const generateECGPoint = (time: number) => {
  ...
  // QRS波群 (0.2-0.25周期)
    ...
    if (qrsPos < 0.2)
      value = -0.2 - Math.random() * 0.1 // Q波
    else if (qrsPos < 0.6)
      value = 0.8 + Math.random() * 0.4 // R波
...
function getCurrentColor() {
  return getComputedStyle(document.documentElement).getPropertyValue('--chart-1').trim()
}
